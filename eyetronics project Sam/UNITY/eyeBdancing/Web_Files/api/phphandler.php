...
{		
	$fb_id = $_POST[ "id" ];
	$eyeBcom_id = $_POST[ "eyeBcom_id" ];
		
	$eyeBcom_query = "SELECT * FROM fb_users WHERE eyeBcom_id = '$eyeBcom_id' OR fb_id = '$fb_id'";		
	$eyeBcom_results = mysql_query( $eyeBcom_query, $link ) or die( mysql_error() );

	if( mysql_num_rows( $eyeBcom_results ) > 0 ) 
	{	
		while( $eyeBcom_result = mysql_fetch_array( $eyeBcom_results ) )
		{
			$record  =       $eyeBcom_result[ 'eyeBcom_id'   	];	// 0
			$record .= ';' . $eyeBcom_result[ 'fb_id'   		];	// 1	
			$record .= ';' . $eyeBcom_result[ 'f_name'  		];	// 2
			$record .= ';' . $eyeBcom_result[ 'pic_url' 		];	// 3
			$record .= ';' . $eyeBcom_result[ 'mesh_url'		];	// 4
			$record .= ';' . $eyeBcom_result[ 'txr_url' 		];	// 5
			$record .= ';' . $eyeBcom_result[ 'score'   		];	// 6
		}
...
{			 
	...
	$query = "UPDATE fb_users SET score = '$score' WHERE fb_id = '$fb_id' OR eyeBcom_id = '$eyeBcom_id'";		
